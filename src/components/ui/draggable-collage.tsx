...
function DraggableCard({ 
  ...
  const blockClickAfterDrag = (e: React.MouseEvent<HTMLDivElement>) => {
    const start = pointerStartRef.current;
    const moved = start && Math.hypot(e.clientX - start.x, e.clientY - start.y) > 5;
  ...
  return (
    <motion.div
      ...
      style={{ ...restStyle, x, y, rotate: initialRotate, scale: initialScale, zIndex }}
    ...
    <div className="draggable-collage-container relative w-full h-[100dvh] overflow-hidden bg-secondary text-foreground select-none" style={{ fontFamily: 'var(--sans)' }}>
      ...
      <div className="mobile-about-panel md:hidden h-full overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden flex flex-col gap-3 px-4 pt-14 pb-8 pointer-events-auto relative z-10">
        ...
        <div className="grid grid-cols-2 gap-3" style={{ fontFamily: 'var(--sans)' }}>
          <div className="bg-card border border-border/60 rounded-xl p-2.5 pb-6 shadow-[0_6px_28px_-6px_rgba(0,0,0,0.18)] -rotate-[2deg]">
            ...
            <p className="mt-2.5 text-micro text-muted-foreground tracking-widest text-center" style={{ fontFamily: 'var(--sans)' }}>Phuc Loc · Saigon</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3" style={{ fontFamily: 'var(--sans)' }}>
          ...
          <div className="bg-foreground text-background rounded-2xl p-3 shadow-xl rotate-[0.5deg] flex flex-col gap-2">
            ...
            <div className="flex justify-between" style={{ fontFamily: 'var(--mono)' }}>
              <span className="text-micro text-background/30">{formatTime(spotifyProgress)}</span>
              <span className="text-micro text-background/30">{formatTime(spotifyTotalSeconds)}</span>
            </div>
          ...
          <div className="-rotate-[1deg]">
            <div className="relative">
              <div className="bg-card border border-border/60 rounded-xl px-4 py-4 shadow-card w-full">
                <p className="text-micro uppercase tracking-widest text-muted-foreground mb-3" style={{ fontFamily: 'var(--sans)' }}>rate this portfolio</p>
        ...
        <div className="grid grid-cols-2 gap-3" style={{ fontFamily: 'var(--sans)' }}>
          ...
          <div className="bg-accent text-accent-foreground rounded-xl px-4 py-4 rotate-[2deg]">
            <p className="text-micro uppercase tracking-widest text-accent-foreground/60 mb-1.5" style={{ fontFamily: 'var(--sans)' }}>currently building</p>
        ...
        <div className="bg-card border border-border/60 rounded-xl px-4 py-3.5 shadow-card -rotate-[0.5deg]">
          <p className="text-micro uppercase tracking-widest text-muted-foreground mb-2.5" style={{ fontFamily: 'var(--sans)' }}>interests</p>
          <div className="flex flex-wrap gap-1.5">
            {["CrossFit", "Music", "Reading", "Nature walks", "New restaurants", "Japanese learner", "AI-first"].map((interest, i) => (
              <span key={i} className="px-2.5 py-1 rounded-full border border-border text-micro text-foreground/70" style={{ fontFamily: 'var(--sans)' }}>{interest}</span>
            ))}
          </div>
        </div>
      ...
      >
        ...
        <div className="absolute inset-0 z-0 flex flex-col items-center justify-center px-8 -translate-y-[4%] pointer-events-none">
          <h2 className="italic text-center whitespace-nowrap font-light" style={{ fontFamily: 'var(--serif)', fontSize: 'var(--fs-display-xl)', lineHeight: '1.08' }}>
            I turn paid traffic into profit.
          </h2>
        </div>
        ...
        >
          <div className="bg-[#4ECCA3] border border-white/40 text-[#0a2e22] rounded-xl px-5 py-5 w-[192px] 3xl:w-[240px] 4xl:w-[280px] shadow-lg">
            ...
            <div className="space-y-1.5" style={{ fontFamily: 'var(--sans)' }}>
              <div className="flex items-center gap-2"><MapPin className="opacity-60" size={12} strokeWidth={1.8} aria-hidden="true" /><span className="text-micro">Ho Chi Minh City</span></div>
              <div className="flex items-center gap-2"><Laptop className="opacity-60" size={12} strokeWidth={1.8} aria-hidden="true" /><span className="text-micro">Remote Vietnam</span></div>
              <div className="flex items-center gap-2"><Globe2 className="opacity-60" size={12} strokeWidth={1.8} aria-hidden="true" /><span className="text-micro">Global remote</span></div>
            </div>
          </div>
        </DraggableCard>
        ...
        >
          <div className="bg-card border border-border/60 rounded-xl p-4 w-[180px] 3xl:w-[230px] 4xl:w-[270px] shadow-sm">
            <p className="text-micro uppercase tracking-widest text-muted-foreground mb-2.5" style={{ fontFamily: 'var(--sans)' }}>currently watching</p>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-micro text-muted-foreground" style={{ fontFamily: 'var(--sans)' }}>82%</span>
            </div>
          </div>
        </DraggableCard>
        ...
        >
          <div className="bg-card border border-border/60 rounded-xl px-5 py-4 shadow-card max-w-[250px] 3xl:max-w-[310px] 4xl:max-w-[360px]">
            <p className="text-micro uppercase tracking-widest text-muted-foreground mb-3" style={{ fontFamily: 'var(--sans)' }}>interests</p>
            <div className="flex flex-wrap gap-1.5">
              {["CrossFit", "Music", "Reading", "Nature walks", "New restaurants", "Japanese learner", "AI-first"].map((interest, i) => (
                <span key={i} className="px-2.5 py-1 rounded-full border border-border text-micro text-foreground/70" style={{ fontFamily: 'var(--sans)' }}>{interest}</span>
              ))}
        ...
        >
          <a href="https://www.linkedin.com/in/phucloc/" target="_blank" rel="noopener noreferrer" draggable="false" className="block border border-white/30 rounded-xl px-5 py-4 w-[190px] 3xl:w-[240px] 4xl:w-[280px] shadow-lg hover:opacity-95 transition-all text-white bg-[#0a66c2]" style={{ fontFamily: 'var(--sans)' }}>
            <p className="text-micro uppercase tracking-widest text-white/60 mb-2">find me online</p>
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs font-medium leading-snug">LinkedIn<br /><span className="text-white/70 text-micro">/in/phucloc</span></p>
            </div>
          </a>
        </DraggableCard>
        ...
        >
          <div className="block bg-[#FFE45C] border border-[#3a2e00]/15 text-[#3a2e00] rounded-xl px-5 py-4 w-[180px] 3xl:w-[230px] 4xl:w-[270px] shadow-lg" style={{ fontFamily: 'var(--sans)' }}>
            <p className="text-micro uppercase tracking-widest opacity-60 mb-2">cv</p>
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs font-medium leading-snug">Resume<br /><span className="opacity-60 text-micro">Web format</span></p>
            </div>
          </div>
        </DraggableCard>
        ...
        >
          <a href="mailto:hi@loc.digital" draggable="false" className="block bg-foreground text-background rounded-xl px-5 py-4 w-[190px] 3xl:w-[240px] 4xl:w-[280px] shadow-lg hover:opacity-95 transition-all" style={{ fontFamily: 'var(--sans)' }}>
            <p className="text-micro uppercase tracking-widest text-background/50 mb-2">open to work</p>
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs font-medium leading-snug">Email me<br /><span className="text-background/60 text-micro">hi@loc.digital</span></p>
            </div>
          </a>
        </DraggableCard>
        ...
        >
          <div className="bg-[#FFF3CD] border border-[#F0C040]/40 text-[#7a5c00] rounded-xl px-4 py-3 w-[170px] 3xl:w-[220px] 4xl:w-[260px] shadow-sm" style={{ fontFamily: 'var(--sans)' }}>
            <p className="text-micro leading-snug">Currently learning<br /><span className="text-xs font-semibold">🇯🇵 Japanese for fun</span></p>
          </div>
        </DraggableCard>
        ...
        >
          <div className="bg-card border border-border/60 rounded-xl px-5 py-4 shadow-card w-fit whitespace-nowrap">
            <p className="text-micro uppercase tracking-widest text-muted-foreground mb-2" style={{ fontFamily: 'var(--sans)' }}>Saigon, VN</p>
            <div className="flex items-end gap-1.5">
              <span className="text-[26px] font-bold leading-none tracking-tight tabular-nums" style={{ fontFamily: 'var(--mono)' }}>
                {timeStr}
              </span>
              <span className="text-micro text-foreground/40 mb-1 uppercase tracking-widest" style={{ fontFamily: 'var(--sans)' }}>{period}</span>
            </div>
            <p className="text-micro text-muted-foreground mt-1.5" style={{ fontFamily: 'var(--mono)' }}>ICT · UTC+7</p>
          </div>
        </DraggableCard>
