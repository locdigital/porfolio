...
export async function POST(request: NextRequest) {
  ...
  if (!name) {
    return NextResponse.json({ success: false, error: "Enter a product name first." }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ success: true, item: fallbackFill(name, sectionTitle) });
  }
}
